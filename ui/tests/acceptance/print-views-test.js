...
module('Acceptance | print views', function (hooks) {
  ...
  test('Viewing /print/reports', async function (assert) {
    ...
    assert
      ...
      .hasText('Kart Wins By Character', 'The report title shows up');
    assert.deepEqual(
  ...
  test('Viewing /print/dashboards', async function (assert) {
    ...
    assert
      .dom('.navi-widget__title')
      .hasText('Kart Wins By Character', 'The widget title shows up');
    assert
      .dom('.c3-axis-y-label')
      .hasText('Wins', 'The widget y-axis title shows up');
    assert.deepEqual(
      findAll('.line-chart-widget .c3-legend-item').map((el) =>
        el.textContent.trim()
      ),
